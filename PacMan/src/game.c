...
void print_board_state(void) {
    if (!g_game_state) {
        printf("Game state not initialized\n");
        return;
    }
    ...
    for (int i = 0; i < BOARD_HEIGHT; i++) {
        for (int j = 0; j < BOARD_WIDTH; j++) {
            switch (g_game_state->board[i][j]) {
                ...
                case CELL_DOT:   printf("."); break;
                case CELL_PLAYER: printf("P"); break;
...
void print_game_statistics(void) {
    ...
    printf("\nGame Statistics:\n");
    printf("Player Position: (%d, %d)\n", g_game_state->player_pos.x, g_game_state->player_pos.y);
    printf("Dots Collected: %d\n", g_game_state->dots_collected);
    printf("Total Dots: %d\n", g_game_state->total_dots);
    printf("Remaining Dots: %d\n", get_remaining_dots());
    printf("Moves Count: %d\n", g_game_state->moves_count);
    printf("Game Over: %s\n", g_game_state->game_over ? "Yes" : "No");
    printf("Game Won: %s\n", g_game_state->game_won ? "Yes" : "No");
}

/* 验证游戏状态（调试用） */
int validate_game_state(void) {
    if (!g_game_state) {
        printf("Error: Game state not initialized\n");
        return 0;
    }
    
    /* 检查玩家位置是否有效 */
    int px = g_game_state->player_pos.x;
    int py = g_game_state->player_pos.y;
    if (!is_within_bounds(px, py)) {
        printf("Error: Player position out of bounds: (%d, %d)\n", px, py);
        return 0;
    }
    
    /* 检查玩家位置是否在棋盘上正确标记 */
    if (g_game_state->board[py][px] != CELL_PLAYER) {
        printf("Error: Player position not marked on board\n");
        return 0;
    }
    
    /* 检查统计数据是否合理 */
    if (g_game_state->dots_collected < 0 || g_game_state->total_dots < 0 || 
        g_game_state->moves_count < 0) {
        printf("Error: Invalid game statistics\n");
        return 0;
    }
    
    if (g_game_state->dots_collected > g_game_state->total_dots) {
        printf("Error: Collected dots exceed total dots\n");
        return 0;
    }
